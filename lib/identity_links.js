...
export async function ensureUserExistsByEmail(email, { queryFn, userIdFactory } = {}) {
  ...
  try {
    ...
    const insertQuery = `
      ...
      VALUES (@user_id, @email, CURRENT_TIMESTAMP(), @metadata)
    `;
    await runQuery({
      query: insertQuery,
      params: {
...
export async function fetchLatestLinkedChatIdByUserId(userId, { queryFn } = {}) {
  ...
  try {
    const query = `
      SELECT chat_id
      FROM ${chatLinks}
      WHERE user_id = @user_id AND status = "LINKED"
      ORDER BY created_at DESC
      LIMIT 1
    `;
...
export async function insertChatLink({ chatId, userId, provider = "google", metadata = null }, { queryFn } = {}) {
  ...
  try {
    const query = `
      ...
      VALUES (@chat_id, @user_id, @provider, "LINKED", CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), @metadata)
    `;
    await runQuery({
      query,
      params: {
